...
namespace {
  void print_keys_if_not_empty(
    ...
    for (auto &k_v : map) {
      GLOG_FAIL(
          "[Key]:", std::string(k_v.first), " [HASH]:", k_v.first.hash_value,
          " [n_refs]:", k_v.second.n_refs, " [Address]: ", k_v.second.buf);
    }
  }
...
namespace flash {
  ...
  void *Cache::get_buf(const flash_ptr<void> &fptr, const StrideInfo &sinfo,
    ...
    if (is_active(k)) {
      ...
      // GLOG_INFO("[HASH]: ", k.hash_value,
      //          " [n_refs]:", this->active_map[k].n_refs,
      //          " [Address]: ", this->active_map[k].buf);
  ...
  void Cache::alloc_bufs(BaseTask *tsk) {
    ...
    for (auto &key : read_only_keys) {
      if (is_active(key)) {
        ...
        // GLOG_DEBUG("[RO]MISS", " [Key]:", std::string(key),
        //          " [HASH]:", key.hash_value, " :QUEUEING");
        add_backlog(key, false, false);
      }
    }
    ...
    for (auto &key : write_only_keys) {
      if (is_active(key)) {
        GLOG_ERROR("write-only-buf in active-map", " [WO]",
                   " [Key]:", std::string(key),
                   " [Address]: ", active_map[key].buf);
      } else if (is_in_io(key)) {
        GLOG_ERROR("write-only-buf in io-map", " [WO]",
                   " [Key]:", std::string(key),
                   " [Address]: ", io_map[key].buf);
      } else if (is_zero_ref(key)) {
        GLOG_ERROR("write-only-buf in zero-ref-map", " [WO]",
                   " [Key]:", std::string(key),
                   " [Address]: ", zero_ref_map[key].buf);
      } else {
        GLOG_DEBUG("[MISS]:", std::string(key), ":QUEUEING");
        add_backlog(key, true, true);
        // GLOG_INFO("[MISS][WO]", " [Key]:", std::string(key),
        //          " [HASH]: ", key.hash_value);
      }
    }
  ...
  void Cache::move_zero_to_active(const Key &k) {
    ...
    // GLOG_INFO("[Move Z->A]", " [Key]:", std::string(k),
    //          " [HASH]: ", k.hash_value, " [n_refs]: ", v.n_refs,
    //          " [Address]: ", v.buf);
  }
  ...
  void Cache::move_io_to_active(const Key &k) {
    ...
    this->active_map[k] = v;
    this->active_map[k].n_refs = 1;
    // GLOG_INFO("[Move IO->A]", " [Key]:", std::string(k),
    //          " [HASH]: ", k.hash_value, "[n_refs]: ", v.n_refs,
    //          "[Address]: ", v.buf);
  }
